...
public class Test {
  ...
  private static int runBasic() {
    ...
    Status.And<String> sv = dbm.compareExchangeAdvanced(
        "xyz", RemoteDBM.ANY_STRING, RemoteDBM.ANY_STRING, 0, false);
    check(sv.status.equals(Status.INFEASIBLE_ERROR));
    check(sv.value == null);
    sv = dbm.compareExchangeAdvanced("xyz", null, "abc", 1, true);
    check(sv.status.equals(Status.SUCCESS));
    check(sv.value == null);
    sv = dbm.compareExchangeAdvanced("xyz", RemoteDBM.ANY_STRING, "def", 1, true);
    check(sv.status.equals(Status.SUCCESS));
    check(sv.value.equals("abc"));
    sv =dbm.compareExchangeAdvanced("xyz", "def", null, 1, true);
    check(sv.status.equals(Status.SUCCESS));
  ...
  private static int runIter() {
    ...
    if (status.equals(Status.SUCCESS)) {
      ...
      check(dbm.count() == 8);
      check(dbm.clear().equals(Status.SUCCESS));
      check(dbm.pushLast("one", 0, false).equals(Status.SUCCESS));
      check(dbm.pushLast("two", 0, true).equals(Status.SUCCESS));
      check(iter.first().equals(Status.SUCCESS));
      status.set(Status.UNKNOWN_ERROR, "");
      ...
      check(Arrays.equals(record[0], new byte[]{0,0,0,0,0,0,0,0}));
      check(Arrays.equals(record[1], "one".getBytes()));
      strRecord = dbm.popFirstString(1, status);
      check(status.equals(Status.SUCCESS));
      check(strRecord[0].equals("\0\0\0\0\0\0\0\1"));
      check(strRecord[1].equals("two"));
      strRecord = dbm.popFirstString(0, status);
      check(status.equals(Status.NOT_FOUND_ERROR));
      check(strRecord == null);
      check(dbm.count() == 0);
    } else {
